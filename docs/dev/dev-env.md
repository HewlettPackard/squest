...
```bash
docker-compose -f docker-compose.yml  -f mysql.docker-composer.yaml -f dev.docker-compose.yml up db phpmyadmin rabbitmq redis-cache
```
If you use postgres SQL, update docker/environment_variables/squest.env, then run
```bash
docker-compose -f docker-compose.yml  -f psql.docker-composer.yaml -f dev.docker-compose.yml up db phpmyadmin rabbitmq redis-cache
```
