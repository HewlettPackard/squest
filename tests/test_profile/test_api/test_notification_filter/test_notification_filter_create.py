...
class TestApiNotificationFilterCreate(BaseTestProfile):

    def setUp(self):
        super(TestApiNotificationFilterCreate, self).setUp()
        self.post_data = {
            'name': "myFilterName",
        }
        self.create_notification_filter_url = reverse('api_notification_filter_list_create')
    ...
    def test_cannot_post_notification_filter_on_another_profile(self):
        ...
        old_count = NotificationFilter.objects.filter(profile=self.superuser.profile).count()
        self._create_notification_filter()
        self.assertFalse(NotificationFilter.objects.exclude(id=self.notification_filter_test_3.id).filter(profile=self.superuser_2.profile).exists())
        self.assertEqual(old_count + 1, NotificationFilter.objects.filter(profile=self.superuser.profile).count())
