...
class TestApiRequestNotificationFilterList(BaseTestRequestNotificationAPI):
    def setUp(self):
        super(TestApiRequestNotificationFilterList, self).setUp()
        self.get_notification_filter_list_url = reverse_lazy('api_requestnotification_list_create')
    ...
    def test_admin_can_get_his_notification_filters(self):
        ...
        self.assertEqual(response.data['count'],
                         RequestNotification.objects.filter(profile=self.superuser.profile.id).count())
