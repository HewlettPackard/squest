...
class TestApiRequestPut(BaseTestRequest):
    ...
    def test_customer_cannot_put_on_non_own_request(self):
        ...
        response = self.client.put(url, data=self.put_data, content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_cannot_put_request_when_loggout(self):
        self.client.logout()
        response = self.client.put(self.get_request_details_url, data=self.put_data, content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
