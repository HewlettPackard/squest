...
class TestApiApprovalWorkflowPut(BaseApproval):

    def setUp(self):
        super(TestApiApprovalWorkflowPut, self).setUp()
        self.put_data = {
            "name": "My new approval workflow",
            "entry_point": self.test_approval_step_1.id,
        }
        self.kwargs = {
        ...
        self.expected_data["entry_point"] = self.test_approval_step_1.id

    def test_admin_put_approval_workflow(self):
        response = self.client.put(self.get_approval_workflow_details_url, data=self.put_data,
                                   content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        check_data_in_dict(self, [self.expected_data], [response.data])

    def test_customer_cannot_put_approval_workflow(self):
        self.client.force_login(user=self.standard_user)
        response = self.client.put(self.get_approval_workflow_details_url, data=self.put_data,
                                   content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_cannot_put_approval_workflow_when_logout(self):
        self.client.logout()
        response = self.client.put(self.get_approval_workflow_details_url, data=self.put_data,
                                   content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
