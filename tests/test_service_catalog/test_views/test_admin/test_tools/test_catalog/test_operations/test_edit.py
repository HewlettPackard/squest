...
class OperationEditTestCase(BaseTest):
    ...
    def test_operation_edit_survey(self):
        ...
        response = self.client.get(url)
        self.assertEqual(200, response.status_code)
        self.assertTrue(self.update_operation_test.survey_fields.get(name="text_variable").is_customer_field)
        self.assertFalse(self.update_operation_test.survey_fields.get(name="multiplechoice_variable").is_customer_field)
        response = self.client.post(url, data=data)
        self.assertEqual(302, response.status_code)
        self.update_operation_test.refresh_from_db()
        self.assertFalse(self.update_operation_test.survey_fields.get(name="text_variable").is_customer_field)
        self.assertTrue(self.update_operation_test.survey_fields.get(name="multiplechoice_variable").is_customer_field)
    ...
    def test_update_survey_when_job_template_on_operation_changed(self):
        ...
        response = self.client.get(url)
        self.assertEqual(200, response.status_code)
        self.assertEqual(0, self.create_operation_empty_survey_test.survey_fields.all().count())
        response = self.client.post(url, data=data)
        self.assertEqual(302, response.status_code)
        ...
        for key in _get_keys_from_survey(self.job_template_test.survey):
            self.assertTrue(self.create_operation_empty_survey_test.survey_fields.filter(name=key).exists())
