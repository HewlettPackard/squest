...
def portfolio_create(request):
    parent_portfolio = request.GET.get('parent_portfolio')
    parent = get_object_or_404(Portfolio.objects.all(), id=parent_portfolio) if parent_portfolio else None
    if request.method == 'POST':
        form = PortfolioForm(request.POST, request.FILES or None, parent_portfolio=parent_portfolio)
        if form.is_valid():
            form.save()
            return redirect(get_portfolio_list_url(parent_portfolio))
    else:
        form = PortfolioForm(parent_portfolio=parent_portfolio)
    breadcrumbs = get_portfolio_breadcrumbs(parent_portfolio)
    breadcrumbs.append({'text': 'Create a portfolio', 'url': ''})
    context = {
        'form': form,
...
def portfolio_edit(request, portfolio_id):
    ...
    parent_portfolio = portfolio.parent_portfolio.id if portfolio.parent_portfolio else None
    form = PortfolioForm(request.POST or None, request.FILES or None, instance=portfolio, parent_portfolio=parent_portfolio)
    if form.is_valid():
        form.save()
        return redirect(get_portfolio_list_url(parent_portfolio))
    breadcrumbs = get_portfolio_breadcrumbs(parent_portfolio)
    breadcrumbs.append({'text': f'Edit {portfolio.name}', 'url': ''})
    context = {
        'form': form,
        'portfolio': portfolio,
        'breadcrumbs': breadcrumbs,
...
def portfolio_delete(request, portfolio_id):
    ...
    if request.method == "POST":
        ...
        return redirect(get_portfolio_list_url(parent_portfolio))
    breadcrumbs = get_portfolio_breadcrumbs(parent_portfolio)
    breadcrumbs.append({'text': f'Delete {portfolio.name}', 'url': ''})
    context = {
        'breadcrumbs': breadcrumbs,
