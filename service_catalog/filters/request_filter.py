...
class RequestFilter(SquestFilter):
    class Meta:
        model = Request
        fields = ['instance__name', 'instance__id', 'user__username', 'instance__service__id', 'operation', 'operation__type',
                  'state']
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.filters['instance__service__id'].field.choices = [(service.id, service.name) for service in Service.objects.all()]
        self.filters['operation'].field.choices = [(operation.id, f"{operation.service.name}-{operation.name}") for operation in Operation.objects.all()]
